make_manage_zones = function(hfp_in,
                             r_ref = base,
                             mask = r_mask,
                             iso3,
                             output_dir = NULL){
  ...
  r = rast(raster::raster(r) | raster::raster(crops) | raster::raster(forests))
  ...
  terra::writeRaster(r,
            glue::glue("{output_dir}/manage_zone_{iso3}.tif"),
            gdal = c("COMPRESS=DEFLATE","OVERVIEWS=NONE"),
            NAflag = 255,
            overwrite = TRUE,
            datatype = "INT1U",
  ...
  # V2 - agriculture areas only
  crops = rast(glue::glue("{output_dir}/crops_{iso3}.tif"))
  crops[crops > 0] = 1
  
  terra::writeRaster(crops,
            glue::glue("{output_dir}/manage_zone_ag_{iso3}.tif"),
            gdal = c("COMPRESS=DEFLATE","OVERVIEWS=NONE"),
            NAflag = 255,
            overwrite = TRUE,
            datatype = "INT1U",

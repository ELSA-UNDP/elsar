...
#'            Other functions, like `mean`, can also be used.
#'
...
#'}
exact_rasterise <- function(features, attribute, pu_layer, fun = sum) {
  r_stack <- terra::rast()
  ...
  if (nrow(features) > 1) {
    ...
    r_stack <- terra::app(r_stack, function(r, ...) fun(r, na.rm = TRUE))
  } else {

...
`elsar_plot_static_raster()` allows plotting both continuous and
discrete data from a `SpatRaster` in `ggplot` or `tmap`. It can be
combined with
...
- data_type:
  ...
  Character. Either "continuous" (default) or "discrete" to specify how
...
A `ggplot` or `tmap` object, or a list with the plot and the data frame
if `raster_df_out = TRUE`.
